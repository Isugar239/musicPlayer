...
public:
    ...
    string *musics = ReadMusicContent(nowTrack+1, db);
    nowMusic = LoadMusicStreamFromMemory(".wav", (unsigned char *)musics->c_str(), musics->size());
        
    }
    void getMusic(nowTrack)
    {
        string *musics = ReadMusicContent(nowTrack, db);
        nowMusic = LoadMusicStreamFromMemory(".wav", (unsigned char *)musics->c_str(), musics->size());
    }
...
class button
{
    ...
    Rectangle pos = {};
    // what it needs to do
    function<void()> void click()
    {
        // do function, what was declarated
    }
};
button updater("update", {1, 2, 3, 4}, UpdateTrack());
// some code...
updater.click() int main()
{
    int musicCounter = 0;
    // Initialization
    //--------------------------------------------------------------------------------------
